...
namespace RDA.AutoRepair.UI
{
    public partial class Form1 : Form
    {
        ...
        {
            int id = customers.GetNextId();
            Customer c = new Customer(id, txtFirstName.Text, txtLastName.Text, txtPhone.Text);
            int rowsAffected = c.InsertIntoDB();
            MessageBox.Show(rowsAffected + " rows affected.");
            customers.Add(c);
            //RebindCustomersRebindCustomers(); // Automatically called by delegate
        }
        ...
        private void btnUpdateCustomer_Click(object sender, EventArgs e)
        {
            if (lstCustomers.SelectedItem is Customer selected)
            {
                ...
                int rowsAffected = selected.UpdateDB();
                MessageBox.Show(rowsAffected + " rows affected.");
        ...
        private void btnDeleteCustomer_Click(object sender, EventArgs e)
        {
            if (lstCustomers.SelectedItem is Customer selected)
            {
                int rowsAffected = selected.DeleteFromDB();
                MessageBox.Show(rowsAffected + " rows affected.");
